...
def main():
	...
	for p in planets:
		...
		while (n < 24):
			if (n < 10):
				ts = '2019-8-27 0%d:00:00' % (n)
			else:
				ts = '2019-8-27 %d:00:00' % (n)
			ts = Time(ts) - utcOffset
			thisAlt = getAlt(p.ra, p.dec, MYLAT, MYLON, MYELEV, ts)
			if thisAlt >= 0: #don't plot points below the horizon
				graphAlt.append(thisAlt)
			else:
				graphAlt.append(0)
			thisAz = getAz(p.ra, p.dec, MYLAT, MYLON, MYELEV, ts)
			graphAz.append(thisAz*(pi/180))
			# For .CSV format...
			# date, hour(24), hour as integer, azimuth*, altitude*)
			#print("%s,%s,%d,%f,%f," % (ts.split()[0], ts.split()[1], n, thisAz, thisAlt) )
			print("%s,%d,%f,%f," % (p.id, n, thisAz, thisAlt) )
			n += 1
